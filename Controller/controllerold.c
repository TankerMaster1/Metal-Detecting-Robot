...
void main(void)
{
	...
	while(1)
	{
		...
		*/
		...
		sendDataToSlave("Right");
		
		}
		else if (adcval_x > 650 && adcval_x < 1100) {
		LCDprint("Left", 1, 1);
		sendDataToSlave("Left");
		...
		else if (adcval_y > 650 && adcval_y < 1100) {
		LCDprint("Down", 1, 1);
		sendDataToSlave("Down");
